<?php
	trait CartModel{		
		 ...
		 * @param int
		 */
		public function cartUpdate($id, $number,$checkout,$checkin) {
		    if($number==0){
		        //xóa sp ra khỏi giỏ hàng
		        unset($_SESSION['cart'][$id]);
		    } else {
		       $_SESSION['cart'][$id]['number'] = $number;
		       $_SESSION['cart'][$id]['checkin'] = DateTime::createFromFormat('Y-m-d',$checkin);
		       $_SESSION['cart'][$id]['checkout'] = DateTime::createFromFormat('Y-m-d',$checkout);
		...
		public function cartCheckOut(){
			...
			foreach($_SESSION["cart"] as $product){
				$query = $conn->prepare("insert into orderdetails set order_id=:order_id, product_id=:product_id, price=:price, quantity=:quantity, checkin=:checkin, checkout=:checkout");
				$query->execute(array("order_id"=>$order_id,"product_id"=>$product["id"],"price"=>$product["price"],"quantity"=>$product["number"],"checkin"=>$product['checkin']->format('Y-m-d'),"checkout"=>$product['checkout']->format('Y-m-d')));
			}
			//xoa gio hang
			unset($_SESSION["cart"]);
		}
